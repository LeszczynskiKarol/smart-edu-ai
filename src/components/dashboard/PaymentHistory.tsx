...
const PaymentHistory: React.FC = () => {
    ...
    const renderPaymentStatus = (status: string) => {
        switch (status) {
            case 'completed':
                return <span className="px-2 py-1 bg-green-100 text-green-800 rounded-full">{t('status.completed')}</span>;
            case 'pending':
                return <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-full">{t('status.pending')}</span>;
            case 'failed':
                return <span className="px-2 py-1 bg-red-100 text-red-800 rounded-full">{t('status.failed')}</span>;
            default:
                return <span className="px-2 py-1 bg-gray-100 text-gray-800 rounded-full">{t('status.unknown')}</span>;
        }
    };
                    ...
                    <CardContent className="p-4">
                        ...
                        {payment.stripeSessionId ? (
                            ...
                            >
                                ...
                                {downloadingInvoice === payment._id ? t('invoice.downloading') : t('invoice.download')}
                            </Button>
                        ) : (
                            <span className="text-gray-400">{t('invoice.unavailable')}</span>
                        )}
                    </CardContent>
                </Card>
            ))}
        ...
        payments.length > 0 ? (
            <div className="flex justify-between items-center mt-4">
                ...
                <span>
                    {t('pagination.page', {
                        current: currentPage,
                        total: Math.ceil(filteredAndSortedPayments.length / paymentsPerPage)
                    })}
                </span>
                <Button
                    onClick={() => paginate(currentPage + 1)}
                                ...
                                <TableBody>
                                    {currentPayments.map((payment) => (
                                        <TableRow key={payment._id}>
                                            ...
                                            <TableCell>{payment.type === 'top_up' ? t('type.topUp') : t('type.order')}
                                            </TableCell>
                                            <TableCell>
                                                {formatPrice(payment.paidAmount, payment.currency)}
                                            </TableCell>
